...
COLOR.getLayerEffectColor = function (forEffect) {
    ...
    try {
        ...
        var layerEffects = layerDesc.getObjectValue(PSClass.LayerEffects);
        var colorFill = layerEffects.getObjectValue(forEffect);
        var color = colorFill.getObjectValue(sTID('color'));
...
COLOR.getSolidFillColor = function () {
    try {
        ...
        var layerDesc = executeActionGet(ref);
        var adjList = layerDesc.getList(sTID('adjustment'));
        var color = adjList.getObjectValue(0).getObjectValue(sTID('color'));
...
COLOR.getSolidStrokeColor = function () {
    try {
        ...
        var layerDesc = executeActionGet(ref);
        var strokeObj = layerDesc.getObjectValue(sTID('AGMStrokeStyleInfo'));
        var strokeStyle = strokeObj.getObjectValue(sTID('strokeStyleContent'));
        var strokeColor = strokeStyle.getObjectValue(sTID('color'));
...
COLOR.createSolidFillLayer = function (color) {
    var desc1 = new ActionDescriptor();
    var ref1 = new ActionReference();
    ...
    var colorObject = COLOR.solidColorToDescriptor(color);
    desc3.putObject(PSKey.Color, colorObject.type, colorObject.descriptor);
...
COLOR.replaceColor = function (colorData) {
    try {
        ...
        /*var docName = "color-" + info.id;
            var newDoc = app.documents.add(200, 200, undefined, docName, undefined, DocumentFill.TRANSPARENT);

            var newColor = new SolidColor();
            newColor.rgb.hexValue = info.content[0].colors[0];
            createSolidFillLayer(newColor);
        ...
        $._ext_CORE.writeToLog('PHXS.jsx-replaceColor()', ex);
    }
};
    ...
    try {
        ...
        /*var docRef = $._ext_CORE.getDocument(docName);
            if (docRef) {
                ...
                return getSolidFillColor();
            }
            return undefined;*/

    } catch (ex) {
        $._ext_CORE.writeToLog('PHXS.jsx-getLayerColor()', ex);
    }
};
...
COLOR.setColor = function (color, historyName) {
    try {
        ...
        $._ext_CORE.writeToLog('PHXS.jsx-setColor()', ex);
    }
};
...
COLOR.setStrokeColor = function (color, historyName) {
    try {
        ...
        $._ext_CORE.writeToLog('PHXS.jsx-setStrokeColor()', ex);
    }
};
...
COLOR.setColorOverlay = function (color, historyName) {
    try {
        ...
        $._ext_CORE.writeToLog('PHXS.jsx-setColorOverlay()', ex);
    }
};
